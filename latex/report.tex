...
All models were developed in Python TensorFlow with model workloads executed on a GPU. For each model, in all
convolutional layers, the pretrained weights were loaded and not further trained. Then, a few architectures with
fully connected layers were connected to the pretrained layers and trained with the training set. All models
were tuned with the hyperparameters in table \ref{tab:hyperparameters}.
\begin{table}[!ht]
  \centering
...
The results in this work shows that ResNet50 might have better potential for transfer learning as the
